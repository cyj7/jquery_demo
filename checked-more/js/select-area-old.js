...
function selectArea(dom, opt, callback){
	...
	this.dom.on('click', '.option-txt', function(){ //添加class
		var tp = $(this).parents(".option-list-item");
...
selectArea.prototype.setSkinLft = function(data, searchVal){ //绑定数据 右侧
	var lftDom = $('<div class="option-select-lft">'+
					...
					'</div>');
	var lftListDom = $('<div class="option-list"></div>');
	var listItemDom = '';
	var itemChild = ''; //子菜单data
	this.dom.addClass("option-select-main"); //添加class
	for(var i=0; i<data.length; i++){
		listItemDom += '<div class="option-list-item">';
		item = data[i];
		...
		if(itemChild){
			...
			for(var j=0; j<itemChild.length; j++){
				listItemDom += '<li class="option-child-item"><label><input type="checkbox" value="'+ itemChild[j].id +'"><span>'+ itemChild[j].title +'</span></label></li>';
			}
			listItemDom += '</ul>';
		}else{
			listItemDom += '<p class="option-item"><label><input type="checkbox" value="'+ item.id +'"><span>'+ item.title +'</span></label></p>';
		}
		listItemDom += '</div>';
	}
...
selectArea.prototype.initFn = function(){ //编辑 初始化
	...
	// console.log(lftDom, chk.length, chkp.length, "initFn")
...
selectArea.prototype.searchFn = function(){
	var _this = this;
